...
# 1. Using `transform()` for element-wise operations #same length transform_batch() for batch processing
ps_df["age_in_10_years"] = ps_df["age"].transform(lambda x: x + 10) #spark under the hood
...
ps_df["salary_category"] = ps_df["salary"].apply(categorize_salary) #different output length hence apply()
